...
public class AnimShopButton extends View {
    protected static final String TAG              = "AnimShopButton";
    ...
    /**
     * add CJR 作者原来是用Region clip下圆的范围, 然后contains判断是否圆的点击, 这里用RectF,
     * 扩大点击范围为矩形
     */
    protected RectF mAddRectF, mDelRectF;
    ...
    //展开 加入购物车动画 (提示语的, 饿了么列表, 只有一个＋,不需要提示语动画了)
    ...
    private boolean mPerformAddClick, mPerformDelClick;
    ...
    public AnimShopButton setReplenish(boolean replenish) {
        ...
        if (isReplenish && null == mReplenishPaint) {
            ...
            mReplenishPaint.setTextAlign(Paint.Align.CENTER);
        }
        return this;
    }
    ...
    private void initDefaultValue(Context context) {
        mGapBetweenCircle = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 34, context
        ...
        mRadius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 12.5f, getResources()
                .getDisplayMetrics());
        mCircleWidth = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1f, getResources()
                .getDisplayMetrics());
        mLineWidth = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2f, getResources()
                .getDisplayMetrics());
        mTextSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14.5f, getResources()
                .getDisplayMetrics());

        mHintText = "加入购物车";
        mHintBgColor = mAddEnableBgColor;
        mHintFgColor = mAddEnableFgColor;
        mHingTextSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 12, context
                .getResources().getDisplayMetrics());
        mHintBgRoundValue = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        switch (wMode) {
            ...
            case MeasureSpec.AT_MOST:
                //不超过父控件给的范围内，自由发挥
                int computeSize = (int) (getPaddingLeft() + mRadius * 2 + mGapBetweenCircle +
                        mRadius * 2 + getPaddingRight() + mCircleWidth * 2);
                wSize = computeSize < wSize ? computeSize : wSize;
                break;
            case MeasureSpec.UNSPECIFIED:
                //自由发挥
                computeSize = (int) (getPaddingLeft() + mRadius * 2 + mGapBetweenCircle + mRadius
                        * 2 + getPaddingRight() + mCircleWidth * 2);
                wSize = computeSize;
                break;
            default:
                break;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        float addLeft = mWidth - getPaddingRight() - 2 * (mRadius + mCircleWidth);
        float addTop = 0;
        float addRight = mWidth;
        float addBottom = mHeight;
        mAddRectF.set(addLeft, addTop, addRight, addBottom);

        float delLeft = 0;
        float delTop = 0;
        float delRight = delLeft + getPaddingLeft() + (mRadius + mCircleWidth) * 2;
        float delBottom = mHeight;
        mDelRectF.set(delLeft, delTop, delRight, delBottom);
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (isReplenish) {
            //  显示补货中
            // 计算Baseline绘制的起点X轴坐标
            float baseX = mWidth * 0.5f;
            // 计算Baseline绘制的Y坐标
            float baseY = (mHeight - mReplenishPaint.descent() - mReplenishPaint.ascent()) * 0.5f;
        ...
        if (!ignoreHintArea && isHintMode) {
            ...
            mRectF.set(
                    mLeft + (mWidth - mRadius * 2) * mAnimExpandHintFraction,
                    mTop,
                    mWidth - getPaddingRight(),
                    mHeight - getPaddingBottom());
            canvas.drawRoundRect(mRectF, mHintBgRoundValue, mHintBgRoundValue, mHintPaint);
            if (isShowHintText) {
                //前景文字
                ...
                float baseX = mWidth * 0.5f;
                // 计算Baseline绘制的Y坐标
                float baseY = (mHeight - mHintPaint.descent() - mHintPaint.ascent()) * 0.5f;
            ...
            mDelPath.reset();
            //  改变圆心的X坐标，实现位移
            mDelPath.addCircle(animOffsetMax * mAnimFraction + mLeft + mRadius, mTop + mRadius,
            ...
            canvas.save();
            //  平移动画
            canvas.translate(mAnimFraction * (mGapBetweenCircle / 2 - mTextPaint
                    .measureText(mCount + "") / 2 + 2 * mRadius), 0);
            ...
            canvas.drawText(mCount + "", mGapBetweenCircle / 2 - mTextPaint.measureText
                    (mCount + "") / 2 + mLeft + mRadius * 2, mTop + mRadius - (mFontMetrics.top +
                    mFontMetrics.bottom) / 2, mTextPaint);
            canvas.restore();
            ...
            //  画"+"
            canvas.drawLine(left + mRadius / 2, mTop + mRadius, left + mRadius / 2 + mRadius,
                    mTop + mRadius, mAddPaint);
            canvas.drawLine(left + mRadius, mTop + mRadius / 2, left + mRadius, mTop + mRadius /
                    2 + mRadius, mAddPaint);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                //  因为这里换到action_up的时候才处理事件
                mPerformAddClick = mAddRectF.contains(x, y);
                mPerformDelClick = mDelRectF.contains(x, y);
                return true;
            case MotionEvent.ACTION_MOVE:
                if (mPerformAddClick && !mAddRectF.contains(x, y)) {
                    //  当滑出范围之外时不再处理点击事件
                    mPerformAddClick = false;
                }
                if (mPerformDelClick && !mDelRectF.contains(x, y)) {
                    //  当滑出范围之外时不再处理点击事件
                    mPerformDelClick = false;
                }
                break;
    ...
    /**
     * 使用时，可以重写`onDelClick()`和` onAddClick()`方法，并在合适的时机回调`onCountAddSuccess()`和`
     * onCountDelSuccess()`以执行动画。
     */
